...
export default function FlashDetails({ data }: { data: EventData }) {
  ...
  const hour = isChristening
    ? data.event.ceremonyHour || data.event.partyHour
    : data.event.partyHour;
  const venue =
    (isChristening ? data.location.church : data.location.reception) ||
  ...
  return (
    <dl className={styles.details}>
      <div>
        ...
        <dd>
          {data.event.date}
          {hour && <span className={styles.hour}>{hour}</span>}
        </dd>
      </div>
      {school && school !== venue && (
        <div>
      ...
      {venue && (
        <div>
          ...
          <dd>
            {venue}
            {data.location.place && data.location.place !== venue && (
              <span className={styles.address}>{data.location.place}</span>
            )}
          </dd>
        </div>
      )}
